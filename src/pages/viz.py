...
if len(dataset_df):
    ...
    # ==== Tabs ====
    tab_raw, tab_desc, tab_overview, tab_class, tab_corr, tab_analysis = st.tabs(
        [
            "🥩 Raw data",
            "📄 Description",
            "🔍 Overview",
    ...
    with tab_overview:
        ...
        df_overview = dataset_df.drop(removed_features, axis=1)
        df_overview = (
    ...
    # ==== Class apparition ====
    with tab_class:
        option_app = st.selectbox("Class to visualize", dataset_df.columns)
        st.plotly_chart(visualize_class_apparition(dataset_df, option_app))

    # ==== Correlation ====
    with tab_corr:
        st.plotly_chart(correlation_matrix(dataset_df))
        st.markdown("---")
        col_corr = st.multiselect(
        ...
        if len(col_corr) == 2:
            st.plotly_chart(plot_correlation(dataset_df, *col_corr))
